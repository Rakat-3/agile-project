...
TOPIC = os.getenv("CAMUNDA_TOPIC", "store-publish-data")
WORKER_ID = os.getenv("WORKER_ID", "python-publish-worker")
...
DB_TABLE = os.getenv("PUBLISH_TABLE", "public.publish_data").strip()
...
def fetch_and_lock():
    payload = {
        ...
        "topics": [{
            ...
            "variables": [
                "publishdate", "offerdeadline", "select", "remarks",
...
def upsert(conn, piid: str, task_id: str, v: dict):
    ...
    INSERT INTO {DB_TABLE} (
        ...
        publishdate, offerdeadline, provider, remarks,
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        offerdeadline=EXCLUDED.offerdeadline,
    ...
    with conn.cursor() as cur:
        cur.execute(sql, (
            ...
            v.get("publishdate"), v.get("offerdeadline"), v.get("select"), v.get("remarks"),
            Json(v)
        ))
    conn.commit()
...
def main():
    ...
    print(f"✅ store-publish worker started | TOPIC={TOPIC} | TABLE={DB_TABLE}", flush=True)
    ...
    while True:
        ...
        try:
            ...
            complete_task(task_id, variables={
                "publishStored": {"value": True, "type": "Boolean"},
                "publishStoredAt": {"value": now_iso, "type": "String"},
            })

            print(f"✅ Stored publish + completed task={task_id}", flush=True)
